...
class TestCondensing(unittest.TestCase):
    ...
    def test_condensing_simple(self):
        self.m._get_samples(self.df_train, parallel=False)
        logging.info("Number of original strategies %d" % len(self.m.encoding))
        self.m.condense_strategies()
        logging.info("Number of condensed strategies %d" % len(self.m.encoding_condensed))
        assert len(self.m.encoding) >= len(self.m.encoding_condensed)
